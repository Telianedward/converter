                    ...
                    if($video9['src_1000'] != '' && $video9['src_1000'] != 'in work' && $video9['src_1000'] != 'complete' && !file_exists(__ROOT__.'/UserFiles/'.$video9["user_id"].'/1000k/'.$videoNameArr[0].'/pl1000.m3u8')) {
                        ...
                        if(count($lines) > 0) {
                            ...
                            // Запускаем генерацию потока 1000К битрейт (16:9) --
                                ...
                                $steps = $steps+1;
                                $sql->_update_steps_shell_tasks($steps, $id);
                                shell_exec("ffmpeg -y -re -i ".__ROOT__.$video9['src_1000']." -codec copy -map 0 -f hls -segment_list '".$dirStreamPl1000k."pl1000.m3u8' -hls_playlist_type vod -segment_list_flags +live -segment_time 1 -segment_clocktime_offset 1 '".$dirStreamPl1000k.$videoNameArr[0]."-%03d.ts' 2>&1");
                                $steps = $steps+1;
                                $sql->_update_steps_shell_tasks($steps, $id);
